...
import type { HonoBindings, MessageData, ScheduledMessage } from './types'
 ...
 * @param rest Rest instance created from createReact function
 * @param now Luxon DateTime
 */
  ...
  for (const {
    ...
    try {
      ...
      await env.DB.prepare(
        ...
        .bind(newAttempts, newAttempts >= 3 ? 'failed' : 'pending', errMsg, id)
        .run()
      return
    }

    console.log(`Message ${id} sent was sent as Discord message ${discordId}.`)
...
/**
 * Send weekly summary of upcoming scheduled messages.
 * @param env Cloudflare Bindings
 * @param rest Rest instance created from createReact function
 * @param now Luxon DateTime
 */
